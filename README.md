...
- N3 기준 회독에는 `JLPT_after_recent_md_nonverbs.md`를 중복 병합하고 전체 359개를 50개 단위 8장으로 구성
- `JLPT_after_recent_md_verbs_only.md`의 139개는 기준 회독과 분리된 `동사 집중 챕터`로 구성
- `data/jlpt_kanji.csv`: N5~N4 한자 모음 CSV, 총 182개
- `data/jlpt_words_data.js`: 브라우저에서 바로 쓰기 위한 단어 데이터
- `data/jlpt_verbs.csv`, `data/jlpt_verbs_data.js`: 동사 집중 챕터 전용 데이터
...
- `data/jlpt_final_data.js`: 벼락치기 영상 3개의 최빈출 단어 51개, 한자 읽기 50개, 문법 문형 28개와 영상별 출처

## 동작

- 홈(`/`)에서 N5, N4, N3, 문법, 모르는 단어 페이지를 선택
- 급수별 페이지(`/N5/`, `/N4/`, `/N3/`)에서 단계 회독, 단어장, 20문제 랜덤 테스트 제공
- N3의 `동사 집중 챕터`는 139개 동사를 한 번에 회독하며 기준 회독의 번호, 누적 범위, 완료 장 수와 분리
- N3 기준 회독은 비동사 신규 항목을 280번 이후에 이어 붙여 1~359번으로 구성
...
- 최종 페이지(`/final/`)는 영상 간 반복 단어, 한자 읽기, 문법을 세 개의 독립 회독으로 제공하고 `다시 볼 것`을 별도로 저장
...
- `scripts/final.js`: 영상 빈출 단어·한자·문법 목록, 카드 회독, 다시 볼 항목 저장
...
최근 비동사 파일을 기준 회독에 병합하고 동사 전용 챕터를 다시 만들 때는 `python3 tools/import_recent_vocab.py`를 실행합니다. 표기 변형의 어느 한 표기와 읽기가 같으면 기존 단어로 인식하며, 반복 실행해도 항목이 중복되지 않습니다.
